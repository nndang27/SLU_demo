...
import numpy as np
import torch
from NLU.model import JointPhoBERT, JointXLMR
...
def get_intent_labels(intent_path):
    return [
        label.strip()
        for label in open(intent_path, "r", encoding="utf-8")
    ]


def get_slot_labels(slot_path):
    return [
        label.strip()
        for label in open(slot_path, "r", encoding="utf-8")
    ]
